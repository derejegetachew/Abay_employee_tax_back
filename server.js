...
db.sequelize
  .sync({ alter: false })
  .then(() => {
    // This will not alter the database schema
    console.log("Database synchronized without altering the schema.");
  })
  .catch((err) => {
...
app.listen(PORT, () => {
  log.Info(
    `Server Running at ${PORT} on ${process.env.running_environment}...`
  );
